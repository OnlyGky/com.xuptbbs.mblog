...
import com.xuptbbs.mblog.base.utils.*;
import com.xuptbbs.mblog.base.utils.BeanMapUtils;
import com.xuptbbs.mblog.base.utils.MarkdownUtils;
import com.xuptbbs.mblog.base.utils.PreviewTextUtils;
import com.xuptbbs.mblog.base.utils.ResourceLock;
import com.xuptbbs.mblog.modules.aspect.PostStatusFilter;
import com.xuptbbs.mblog.modules.data.PostVO;
import com.xuptbbs.mblog.modules.data.UserVO;
import com.xuptbbs.mblog.modules.entity.*;
import com.xuptbbs.mblog.modules.entity.*;
import com.xuptbbs.mblog.modules.event.EventTagScore;
import com.xuptbbs.mblog.modules.event.PostUpdateEvent;
import com.xuptbbs.mblog.modules.repository.ResourceRepository;
import com.xuptbbs.mblog.modules.repository.PostAttributeRepository;
import com.xuptbbs.mblog.modules.repository.PostResourceRepository;
...
//					builder.equal(root.get("featured").as(Integer.class), Consts.FEATURED_DEFAULT));
	...
	public long post(PostVO post) {
        ...
        tagCreateService.UpdateScoreByPostId(po.getId(), po.getAuthorId(), EventTagScore.CREATE,
				0);
        userRecScoreService.SaveOrUpdateUserRecScore(po.getAuthorId(), 2);
        try {
            synchronized (lock){
	...
	public PostVO get(long id) {
		...
		if (po.isPresent()) {
			 ...
			 * 记录当前用户的观看积分
			 */
			tagLikeService.UpdateScoreByPostId(id, 0, EventTagScore.LOOK, d.getAuthorId());
			/**
			 * 记录文章作者的被观看积分
			 */
			tagCreateService.UpdateScoreByPostId(id, 0, EventTagScore.LOOK, d.getAuthorId());
			userRecScoreService.SaveOrUpdateUserRecScore(d.getAuthorId(), 1);
			return d;
		}
		return null;
	}
...
//			md5s.add(key.substring(0, key.lastIndexOf(".")));
	...
	@Override
	public Page<PostVO> pagingDraftByAuthorId(Pageable pageable, long userId){

		Page<Post> page = postRepository.findAllByAuthorIdAndStatus(pageable, userId, 1);
		return new PageImpl<>(toPosts(page.getContent()), pageable, page.getTotalElements());
	}
}
